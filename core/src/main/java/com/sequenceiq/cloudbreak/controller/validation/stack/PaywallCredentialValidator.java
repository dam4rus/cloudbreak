...
public class PaywallCredentialValidator {
    ...
    private StackInfo getStackInfo(String stackType, String stackVersion) {
        return "HDP".equals(stackType) ? defaultHDPEntries.getEntries().get(stackVersion) : defaultHDFEntries.getEntries().get(stackVersion);
    }
    ...
    private boolean isDefaultStackRepository(StackInfo hdpInfo, AmbariStackDetailsJson stackDetails) {
        ...
        return stack.containsValue(stackDetails.getRepositoryVersion()) && stack.containsValue(stackDetails.getVersionDefinitionFileUrl());
    }
